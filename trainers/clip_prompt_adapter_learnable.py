...
class Learnable_rate(nn.Module):
    ...
    def forward(self, x1, x2):
        x = torch.cat((x1, x2), dim=1) # Concatenate input vectors along dimension 1
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image):
        ...
        ratio = self.learnable_rate(image_features, text_features)
...
class CLIP_Prompt_Adapter_Learnable(TrainerX):
    ...
    def build_model(self):
        ...
        total_count = sum([np.prod(p.size()) for p in self.model.parameters()])
        print(total_count)
        ...
        self.register_model("clip_prompt_adapter", self.model, self.optim, self.sched)
